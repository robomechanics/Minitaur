...
void Walk::update() {
  ...
  for (int i=0; i<4; ++i) {
    ...
    if ((stepLeg == -1 && (flightLeg == i || flightLeg + i == 3)) || (stepLeg == i)) {
      ...
      leg[i].setGain(EXTENSION, kExtPFlight);
      ...
      leg[i].setGain(ANGLE, kAngPFlight);//(frac < 0.5) ? kAngPFlight : 0.1);
      ...
      leg[i].setOpenLoop(EXTENSION, kExtPStance*(extDes - ext) + rollCtrl + pitchCtrl + kOffset);
    }
    // liftoff 
